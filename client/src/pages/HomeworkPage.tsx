...
export default function HomeworkPage() {
  ...
  const send = async () => { if (!user) { window.location.href = "/login"; return; } if (!file) return toast.error("กรุณาเลือกรูปการบ้านก่อน"); const reader = new FileReader(); reader.onload = () => submit.mutate({ lessonId, fileName: file.name, contentType: file.type, dataUrl: String(reader.result), note }); reader.readAsDataURL(file); };
